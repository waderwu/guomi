...
__constant__ const u4 Sbox_T[256] = {
...
__constant__ const u4 Sbox_T8[256] =
{
...
__constant__ const u4 Sbox_T16[256] =
{
...
__constant__ const u4 Sbox_T24[256] =
{
...
{
	register u4 B0 = load_be<u4>(in, 0);
	register u4 B1 = load_be<u4>(in, 1);
	register u4 B2 = load_be<u4>(in, 2);
	register u4 B3 = load_be<u4>(in, 3);
...
__global__ void speed_test(int n, const u1 *in, u1 *out, const u4 *rkey)
{
  ...
  for (int i=index; i<n; i +=stride)
    SM4_enc_block_cuda(in+16*i, out+16*i,rkey);
}
