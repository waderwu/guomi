...
void SM4_key_schedule(const u1 key[SM4_KEY_SIZE], u4 rkey[SM4_RND_KEY_SIZE / sizeof (u4)])
{
    ...
    K[0] = load_be(key, 0) ^ FK[0];
    K[1] = load_be(key, 1) ^ FK[1];
    K[2] = load_be(key, 2) ^ FK[2];
    K[3] = load_be(key, 3) ^ FK[3];
...
void openmp_sm4_encrypt(const uint8_t *plain, const uint8_t *key, uint8_t *cipher, uint n_block)
{
    ...
    {
        SM4_enc_block(plain+16*j, cipher+16*j, rkey);
    }
...
void openmp_sm4_decrypt(uint8_t *plain, const uint8_t *key, const uint8_t *cipher, uint n_block)
{
    ...
    for (int j=0; j<n_block; j++)
    {
        SM4_dec_block(cipher+16*j, plain+16*j, rkey);
    }
